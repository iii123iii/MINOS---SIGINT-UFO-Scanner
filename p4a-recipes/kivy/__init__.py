...
class KivyRecipe(PyProjectRecipe):
    ...
    # Upstream pins cython<=3.0.12, but Cython 3.0.x does not support Python
    # 3.14 (which the p4a develop python3 recipe builds): its generated C calls
    # removed/changed CPython internals such as _PyList_Extend,
    # _PyGen_SetStopIterationValue and _PyUnicode_FastCopyCharacters, causing
    # "undeclared function" / "too few arguments" compile errors. Cython 3.1.x
    # emits 3.14-compatible shims, so require it here.
    hostpython_prerequisites = ["cython>=3.1.2,<3.2.0"]
