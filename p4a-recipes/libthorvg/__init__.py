...
class LibThorVGRecipe(MesonRecipe):
    ...
    def build_arch(self, arch):
        ...
        with current_directory(build_dir):
            ...
            # The clang runtime lives under a version-numbered directory, and the
            # parent dir differs between NDK layouts: r25 and earlier use
            # "lib64/clang/...", while r26+ use "lib/clang/...". The upstream
            # recipe only globs "lib/clang/*", which yields an empty match (and an
            # IndexError) on NDK r25b. Try both layouts so the recipe works
            # regardless of the pinned NDK version.
            llvm_dir = self.ctx.ndk.llvm_prebuilt_dir
            clang_lib_dir = None
            for libroot in ("lib", "lib64"):
                pattern = join(llvm_dir, libroot, "clang", "*", "lib", "linux", lib_arch)
            ...
            if clang_lib_dir is None:
                raise RuntimeError(
                    "Could not locate the clang runtime lib dir (libomp.so) under "
                    f"{llvm_dir} for arch {lib_arch}. Looked in lib/clang/* and "
                    "lib64/clang/*."
                )
